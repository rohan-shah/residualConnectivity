...
{
	template<class T> void constructMatricesInternal(typename T::LargeIntMatrix& outputTransitionMatrix, FinalColumnVector& outputFinal, InitialRowVector& outputInitial, int gridDimension, const std::vector<unsigned long long>& states, std::size_t& nonZeroCount)
	{
		nonZeroCount=0;
		...
		generateBinaryMasks(states, binaryMasks, gridDimension);
		...
		generateStateGroupBinaryMasks(states, stateGroupBinaryMasks, gridDimension);
		...
		for(unsigned long long binaryEncoding = 1ULL; binaryEncoding < finalBinaryEncoding; binaryEncoding++)
		{
			...
			generateCachedIndices(binaryEncoding, gridDimension, cachedIndices, nGroups, states);
...
skipThisState:
	...
	{
		constructMatricesInternal<constructMatricesDense_impl>(outputTransitionMatrix, outputFinal, outputInitial, gridDimension, states, nonZeroCount);
	}
	...
	{
		constructMatricesInternal<constructMatricesSparse_impl>(outputTransitionMatrix, outputFinal, outputInitial, gridDimension, states, nonZeroCount);
	}
}
