...
namespace residualConnectivity
{
	void averageComponentSize(averageComponentSizeArgs& args)
	{
		...
		const std::vector<mpfr_class>& probabilities = contextObj.getOperationalProbabilities();
		...
		for(int i = 0; i < args.n; i++)
		{
			...
			{
				componentSizes[args.scratchMemory[vertexCounter]]++;
			}
			for(int vertexCounter = 0; vertexCounter < (int)nVertices; vertexCounter++)
			{
				integerSum[vertexCounter] += componentSizes[args.scratchMemory[vertexCounter]];
			}
		}
		int n = args.n;
		args.result.clear();
		std::transform(integerSum.begin(), integerSum.end(), std::back_inserter(args.result), [n](unsigned long long x){return mpfr_class(x)/mpfr_class(n);});
	}
}
