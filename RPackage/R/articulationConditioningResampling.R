articulationConditioningResampling <- function(probability, n, seed, graph, initialRadius)
{
	...
	if(class(graph) == "igraph")
	{
		...
		start <- Sys.time()
		estimate <- .Call(paste0("articulationConditioningResampling_igraph"), graph, vertexCoordinates, probability, n, initialRadius, seed, PACKAGE="residualConnectivity")
		end <- Sys.time()
	}
	else if(class(graph) == "graphNEL")
	{
		...
		start <- Sys.time()
		estimate <- .Call(paste0("articulationConditioningResampling_graphNEL"), graph, vertexCoordinates, probability, n, initialRadius, seed, PACKAGE="residualConnectivity")
		end <- Sys.time()
	}
	else if(class(graph) == "graphAM")
	{
		...
		start <- Sys.time()
		estimate <- .Call(paste0("articulationConditioningResampling_graphAM"), graph, vertexCoordinates, probability, n, initialRadius, seed, PACKAGE="residualConnectivity")
		end <- Sys.time()
	}
	else
	{
