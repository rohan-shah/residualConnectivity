context("check recursiveVarianceReduction function")
library(graph)
...
test_that("Function can be called using graphNEL, graphAM and igraph, and gives same results", 
	{
		...
		for(index in 1:length(probabilities))
		{
			...
			igraphResult <- recursiveVarianceReduction(igraph, probabilities = probability, n = 1000, seed = index)@estimate
			graphAMResult <- recursiveVarianceReduction(graphAM, probabilities = probability, n = 1000, seed = index)@estimate
			graphNELResult <- recursiveVarianceReduction(graphNEL, probabilities = probability, n = 1000, seed = index)@estimate
...
test_that("Test that recursiveVarianceReduction gives numerically accurate results", 
	{
		...
		{
			probability <- probabilities[index]
			igraphResult <- recursiveVarianceReduction(igraph, probabilities = probability, n = 200000, seed = index)@estimate

			exactResult <- as.double(exactRCR(grid4Counts, probability))

			expect_equal(igraphResult, exactResult, tolerance = 0.004)
		}
	})
test_that("Values agree with exact values for small grids and non-uniform probabilities",
{
	...
	for(index in 1:nrow(probabilities))
	{
		...
		methodResult <- recursiveVarianceReduction(graph=graph, n = 100000, seed = 1, probabilities = probabilityVector)
