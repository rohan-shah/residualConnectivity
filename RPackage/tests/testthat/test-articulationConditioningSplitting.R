...
test_that("Function can be called using graphNEL, graphAM and igraph, and gives same results", 
	{
		...
		for(gridSize in gridSizes)
		{
			...
			for(probability in probabilities)
			{
				...
				result_igraph <- articulationConditioningSplitting(graph=igraph, n = 1000, seed = gridSize, initialRadius = 3, probabilities = probability, splittingFactors = rep(1L, 3))
				result_graphNEL <- articulationConditioningSplitting(graph=graphNEL, n = 1000, seed = gridSize, initialRadius = 3, probabilities = probability, splittingFactors = rep(1L, 3))
				result_graphAM <- articulationConditioningSplitting(graph=graphAM, n = 1000, seed = gridSize, initialRadius = 3, probabilities = probability, splittingFactors = rep(1L, 3))
...
test_that("Values agree with exact values for small grids and non-uniform probabilities",
{
	...
	for(index in 1:nrow(probabilities))
	{
		...
		methodResult <- articulationConditioningSplitting(graph=graph, n = 100000, seed = 1, probabilities = probabilityVector, initialRadius = 3, splittingFactors = rep(1L, 3))
