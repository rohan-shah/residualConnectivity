...
namespace discreteGermGrain
{
	int main(int argc, char** argv)
	{
		...
		std::cout << "Expected vertex count is ";

		mpfr_class probability = 0;
		mpfr_class expectedCount = 0;
		for(std::size_t i = 0; i < nVertices+1; i++)
		{
			...
			probability += graphCounts[i] * opProbabilityPower * inopProbabilityPower;
			expectedCount += i*graphCounts[i] * opProbabilityPower * inopProbabilityPower;
		}
		expectedCount /= probability;
		std::cout << expectedCount.str() << std::endl;
		return 0;
	}
}
int main(int argc, char** argv)
